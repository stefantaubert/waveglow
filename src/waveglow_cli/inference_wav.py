...
def init_synthesis_wav_parser(parser: ArgumentParser) -> None:
  parser.description = "Synthesize audio files to audio files (.wav)."
  ...
  parser.add_argument('folder', type=parse_existing_directory, metavar="FOLDER",
                      help="path to folder which contains the audio files that should be synthesized")
...
def infer_wavs(ns: Namespace) -> bool:
  ...
  for wav_path in all_wav_files:
    ...
    if ns.include_stats:
      ...
      save_results(validation_entry_output, output_directory / wav_path.stem)
